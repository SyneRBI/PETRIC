...
from SIRF_data_preparation.evaluation_utilities import read_objectives, get_metrics, plot_metrics, pass_index
...
#%%
qm = QualityMetrics(reference_image, data.whole_object_mask, data.background_mask,
            tb_summary_writer= 'outdir/test',
...
plt.savefig(PETRICDIR / 'output' / (scanID + '_image_at_0.01_0.005.png'))

plt.figure()
data_QC.plot_image(image-data.OSEM_image, **slices, vmin=-cmax/20, vmax=cmax/20)
plt.savefig(PETRICDIR / 'output' / (scanID + '_OSEM_diff_image_at_0.01_0.005.png'))
plt.figure()
data_QC.plot_image(image - reference_image, **slices, vmin=-cmax/100, vmax=cmax/100)
plt.savefig(PETRICDIR / 'output' / (scanID + '_ref_diff_image_at_0.01_0.005.png'))
